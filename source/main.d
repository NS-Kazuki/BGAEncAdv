...
private{
  ...
  immutable WIDTH_MIN = 256;
  immutable HEIGHT_MIN = 256;
  immutable WIDTH_MAX = 65536;
  immutable HEIGHT_MAX = 65536;
  immutable FPS_MAX = 360;

  AVPacket packet;

  alias write_func = void function(AVFrame*, ref LayerData[4], ref color[4], uint, uint);
  ...
  *
  */
  void save_frame(BMS_data b, ref ImageBitmap[] images, ref Moviedata[size_t] movies, ref AudioData audio, ref EncodeData option, ref BGAMode[] modes){
    AVCodec* codec;
    AVCodecContext* context;
    AVFormatContext* format;
  ...
  *
  */
  void copy_bitmap(T : trimming[])(AVFrame* picture, ref ImageBitmap[] images, ref Moviedata[size_t] movies, ref T trimmings, ref LayerData[4] layer, ref EncodeData option, ref BGAMode[] modes){
    color[4] bit;
    int[4] xx;
    int[4] yy;
    Rect[4] rects;
    ...
    foreach(size_t i; 0..4){
      if(layer[i].layer > 0 && ((option.layer <= 1 && i == 3) || (option.layer > 0 && i != 3))){
        if(modes[layer[i].layer] & BGAMode.TRIMMING){
          if(modes[trimmings[layer[i].layer].number] & ~BGAMode.TRIMMING){
            writemode |= (1 << i);
            ...
            if(modes[trimmings[layer[i].layer].number] & BGAMode.MOVIE){
        ...
        }else if(modes[layer[i].layer] == BGAMode.MOVIE){
          debug{
        ...
        }else if(modes[layer[i].layer] == BGAMode.IMAGE){
          debug{
  ...
  void load_pixel(T : trimming[])(AVFrame* picture, ref ImageBitmap[] images, ref Moviedata[size_t] movies, ref T trimmings, uint layer, ref EncodeData option, ref BGAMode[] modes, ref color bit, ref Rect rect, uint xx, uint yy){
    if(modes[layer] & BGAMode.TRIMMING){
      if(modes[trimmings[layer].number] & BGAMode.MOVIE){
    ...
    }else if(modes[layer] == BGAMode.MOVIE){
    ...
    }else if(modes[layer] == BGAMode.IMAGE){
      if(yy >= rect.top && yy < rect.bottom && xx >= rect.left && xx < rect.right){
  ...
  void _write_pixel(int pictmode)(AVFrame* picture, ref LayerData[4] layer, ref color[4] bit, uint x, uint y){
  ...
  *
  */
  void load_images(BMS_data b, ref EncodeData option, ref ImageBitmap[] images, ref Moviedata[size_t] movies, ref BGAMode[] modes){
    string imagefile;
    //
  ...
  *
  */
  void change_movie(T : trimming[])(uint before_id, uint after_id, ref Moviedata[size_t] movies, ref T trimmings, ref BGAMode[] modes, fractionL time){
    if(modes[before_id] & BGAMode.TRIMMING){
      if(modes[after_id] & BGAMode.TRIMMING){
        if(trimmings[after_id].number != trimmings[before_id].number){
          if(modes[trimmings[before_id].number] & BGAMode.MOVIE){
            stop_movie(movies[trimmings[before_id].number]);
          }
          if(modes[trimmings[after_id].number] & BGAMode.MOVIE){
        ...
        if(modes[after_id] != trimmings[before_id].number){
          ...
          if(modes[after_id] & BGAMode.MOVIE){
            start_movie(movies[after_id], time);
          }
        }
      }
    }else if(modes[before_id] & BGAMode.MOVIE){
      if(modes[after_id] & BGAMode.TRIMMING){
  ...
  *
  */
  void get_movie_size(ref BMS_data b, ref EncodeData option, ref ImageBitmap[] images, ref Moviedata[size_t] movies, ref BGAMode[] modes){
    foreach(ref int_delta background; b.background_list){
      if(background.type == Object_type.BGA_BASE ||
        ...
        if(modes[background.value] & BGAMode.TRIMMING){
          if(b.bga_list[background.value].number & ~BGAMode.TRIMMING){
        ...
        }else if(modes[background.value] == BGAMode.MOVIE){
        ...
        }else if(modes[background.value] == BGAMode.IMAGE){
...
int main(string[] agev){
  ...
  if(agev.length > 1 && agev[1] == "--help"){
    ...
    if(exists(option.infile)){
      ...
      if(option.width < WIDTH_MIN){
        option.width = WIDTH_MIN;
      }else if(option.width > WIDTH_MAX){
        option.width = WIDTH_MAX;
      }
      if(option.height < HEIGHT_MIN){
        option.height = HEIGHT_MIN;
      }else if(option.height > HEIGHT_MAX){
        option.height = HEIGHT_MAX;
      }
      if(option.top > (HEIGHT_MAX - option.height) / 2){
        option.top = (HEIGHT_MAX - option.height) / 2;
      }
      if(option.framerate > FPS_MAX){
        option.framerate = FPS_MAX;
      }
      //
      if(option.audiofile != ""){
